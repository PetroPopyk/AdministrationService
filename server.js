...
app.get('/AdministrationUsers', function (req, res) {
    knex.select().from('AdministrationUsers').then(function (AdministrationUsers) {
        res.status(200).send(AdministrationUsers)
    })
});
//Отримати вчителів
app.get('/teachers', function (req, res) {
    knex.select().from('teachers').then(function (teachers) {
...
app.post('/find-teach', function (req, res) {
    knex('pupils').where('name', req.body.name).then(function () {
        res.sendStatus(200);
    })
});
//Видалити вчителя
app.post('/del-teach', function (req, res) {
    knex('teachers').where('id', req.body.id).del().then(function () {
        res.sendStatus(200);
    })
});
//Отримати класи
app.get('/classroom', function (req, res) {
    knex.select('classroom.id as id', 'classroom.name as class', 'teachers.name as name', 'teachers.sname as sname').from('classroom').innerJoin('teachers', 'classroom.teachers_id', 'teachers.id').then(function (classroom) {
...
app.post('/del-class', function (req, res) {
    knex('classroom').where('id', req.body.id).del().then(function () {
        res.sendStatus(200);
    })
});
//Додати стовпчик
app.post('/add-column', function (req, res) {
    knex.raw('ALTER TABLE `sqlkn15_3_ppr`.`teachers` ADD COLUMN ' + req.body.name + ' VARCHAR(45) NOT NULL AFTER `sname`;').then(function () {
        res.sendStatus(200);
    })
});
//Отримати учнів
app.post('/pupils', function (req, res) {
    knex.select().from('pupils').where('classroom_id', req.body.id).then(function (pupils) {
